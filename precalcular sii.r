...
# dependencia rubro ----
trabajadores_dependencia_rubro <- datos_sii$trabajadores_act %>%
  filter(#rubro == rubro_elegido,
    #año == 2019,
    género == "total",
    tipo == "dependencia",
    calculo == "total") %>%
  group_by(rubro, dependencia) %>%
  summarize(trabajadores = sum(valor, na.rm = T))

#dependencia subrubro ----
trabajadores_dependencia_subrubro <- datos_sii$trabajadores_act %>%
  filter(#rubro == rubro_elegido,
    #año == 2019,
    género == "total",
    tipo == "dependencia",
    calculo == "total") %>%
  group_by(rubro, subrubro, dependencia) %>%
  summarize(trabajadores = sum(valor, na.rm = T))
...
tramos_comuna_13 <- datos_sii$tramos_comuna_13 %>%
  ungroup() %>%
  select(año, comuna, tramo, empresas) %>%
  mutate(tramo4 = stringr::str_remove_all(tramo, " \\d")) %>%
  mutate(tramo = recode(tramo, "Sin Ventas/Sin Información" = "Desconocido")) %>%
  mutate(tramo = forcats::fct_relevel(tramo,
...
tramos_region_13 <- datos_sii$tramos_comuna_13 %>%
  ungroup() %>%
  group_by(año, tramo) %>%
  summarize(empresas = sum(empresas, na.rm=T)) %>%
  select(año, tramo, empresas) %>%
  mutate(tramo4 = stringr::str_remove_all(tramo, " \\d")) %>%
  mutate(tramo = recode(tramo, "Sin Ventas/Sin Información" = "Desconocido")) %>%
  mutate(tramo = forcats::fct_relevel(tramo,
                                      "Micro 1", "Micro 2", "Micro 3",
...
tramos_rubro_13 <- datos_sii$tramos_rubro_13 %>%
  ungroup() %>%
  select(año, rubro, tramo, empresas) %>%
  mutate(tramo4 = stringr::str_remove_all(tramo, " \\d")) %>%
  mutate(tramo = recode(tramo, "Sin Ventas/Sin Información" = "Desconocido")) %>%
  mutate(tramo = forcats::fct_relevel(tramo,
                                      "Micro 1", "Micro 2", "Micro 3",
...
#tramos subrubro ----
tramos_subrubro_13 <- datos_sii$tramos_subrubro_13 %>%
  ungroup() %>%
  select(año, rubro, subrubro, tramo, empresas) %>%
  mutate(tramo4 = stringr::str_remove_all(tramo, " \\d")) %>%
  mutate(tramo = recode(tramo, "Sin Ventas/Sin Información" = "Desconocido")) %>%
  mutate(tramo = forcats::fct_relevel(tramo,
                                      "Micro 1", "Micro 2", "Micro 3",
...
datos <- list("empresas_año_rubro_comuna" = empresas_año_rubro_comuna, #datos_sii$empresas
              ...
              "trabajadores_dependencia_rubro" = trabajadores_dependencia_rubro,
              "trabajadores_dependencia_subrubro" = trabajadores_dependencia_subrubro,
              ##
              ...
              "tramos_region_13" = tramos_region_13,
