...
descargar_portalinmobiliario <- function(direccion, tipo = "departamentos", fecha = lubridate::today()) {
  ...
  ruta <- paste0("/mnt/volumen/Dataemprende/scrapping/portalinmobiliario/paginas/", fecha, "/")
  ...
  tiempo_fin <- Sys.time()
  Sys.sleep((tiempo_fin - tiempo_inicio) * 2) #misericordia
...
scrapping_portalinmobiliario <- function(fecha = lubridate::today()) {
  archivos_s <- list.files(paste0("/mnt/volumen/Dataemprende/scrapping/portalinmobiliario/paginas/", fecha))
  ...
  for (archivo in archivos_s) {
    ...
    #cargar archivo
    cat(archivo, fill=T)
    session <- rvest::read_html(paste0("/mnt/volumen/Dataemprende/scrapping/portalinmobiliario/paginas/", fecha, "/", archivo))
...
#guardar
save(base_portal, file = paste0("/mnt/volumen/Dataemprende/scrapping/portalinmobiliario/bases/base_portal_", lubridate::today(), ".rdata"))
...
archivos_y <- list.files("/mnt/volumen/Dataemprende/scrapping/portalinmobiliario/bases/")
...
for (arch in archivos_y) {
  bases_portal[[arch]] <- get(load(paste0("/mnt/volumen/Dataemprende/scrapping/portalinmobiliario/bases/", arch)))
}
