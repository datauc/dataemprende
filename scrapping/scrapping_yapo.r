...
descargar_yapo <- function(paginas = 1:10, espera = 1.5, fecha = lubridate::today()) {
  ...
  for (c in categorias) { #categorías por scrappear
    for (p in paginas) { #páginas de resultados por scrappear
      ...
      ruta <- paste0("/mnt/volumen/Dataemprende/scrapping/yapo.cl/paginas/", fecha, "/")
...
archivos_y <- list.files("/mnt/volumen/Dataemprende/scrapping/yapo.cl/bases/")
...
for (arch in archivos_y) {
  bases_yapo[[arch]] <- get(load(paste0("/mnt/volumen/Dataemprende/scrapping/yapo.cl/bases/", arch)))
}
